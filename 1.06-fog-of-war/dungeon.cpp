...
{
    d->monster_list = (npc_t *) malloc(d->num_monsters * sizeof(npc_t));
...
void print_monster_list(dungeon_t *d, int offset)
{
    ...
    for(i = 0 + offset, j = 0; i < d->num_monsters && j < DUNGEON_Y; i++, j++) {
        const char *y_dir = (d->monster_list[i].pos_from_pc[dim_y] > 0) ? "Units North" : "Units South";
        const char *x_dir = (d->monster_list[i].pos_from_pc[dim_x] > 0) ? "Units West" : "Units East";
