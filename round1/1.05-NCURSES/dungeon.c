...
#include "heap.h"
#include "event.h"
#include "endian.h" // included because Mac doesn't have this by default
#include "npc.h"
...
void create_stairs(dungeon_t *d) {
  int i, x = 0, y = 0, random = 0;
  int x_len = 0, y_len = 0;
  srand((unsigned) NULL);

  for(i = 0; i < d->num_rooms; i++) {
    y_len = d->rooms[i].size[dim_y] / 2;
    x_len = d->rooms[i].size[dim_x] / 2;
    y = d->rooms[i].position[dim_y] + y_len;
    x = d->rooms[i].position[dim_x] + x_len;

    random = rand() % 5;
    if(random == 0) {
      random = rand() % 2;
      if(random == 0) d->map[y][x] = ter_floor_stair_up;
      else d->map[y][x] = ter_floor_stair_down;
    }
  }
}
...
void render_dungeon(dungeon_t *d)
{
  pair_t p;
  clear();
  int pc_y, pc_x, random;
  int y_pos = 1, x_pos = 0;
  pc_y = d->pc.position[dim_y];
  pc_x = d->pc.position[dim_x];
  d->camera_pos[dim_y] = d->pc.position[dim_y];
  d->camera_pos[dim_x] = d->pc.position[dim_x];
  srand((unsigned) NULL);
